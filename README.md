...
| Variation | Config | Rake |
|---|---|---|
| 28 | `passenger_config01` | Axsellya Express — `streakcoaster` + 2× `streakc` + `streakcab` |
| 29 | `passenger_config02` | Brown Streak — `streak` + 3× `streakcoastercab` |
| 30 | `freight_config01` | unused |
...
- **`.ymap` station builds** (Sandy Shores, Paleto, La Mesa, Davis Quartz, ticket
  machine) and their props. The DPS map already has station geometry from other
  resources.
- **`gta5.meta` / `replace_level_meta`** — with the ymaps gone there is no level
  to replace, and declaring it produced
  `Could not find requested level (resources:/…/gta5)` on every client.
